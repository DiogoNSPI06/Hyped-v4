...
module.exports.run = async (client, message, args, prefix, color, config) => { 
  ...
  .addField('> ⚙️| API Info', `[Clique Aqui](https://v4.hypeds.com/api/users/${member.id})`)
  ...
  .setEmoji('◀️')
  .setCustomId(`ArrowLeft_${member.id}_${message.guild.id}`)
  .setStyle('PRIMARY');

  const bArrowLeftDisabled = new Disbut.MessageButton()
  .setEmoji('◀️')
  .setCustomId('ArrowLeftDisabled')
  .setStyle('PRIMARY')
  .setDisabled(true);

  const bArrowRight = new Disbut.MessageButton()
  .setEmoji('▶️')
  .setCustomId(`ArrowRight_${member.id}_${message.guild.id}`)
  .setStyle('PRIMARY');

  const bArrowRightDisabled = new Disbut.MessageButton()
  .setEmoji('▶️')
  .setCustomId('ArrowRightDisabled')
  .setStyle('PRIMARY')
  .setDisabled(true);
