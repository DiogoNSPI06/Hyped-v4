  ...
  Jimp.read(`https://imgur.com/yF4Mbkf.png`, function (err, image) {
    ...
    Jimp.loadFont(Jimp.FONT_SANS_64_BLACK).then(function (font) {
      ...
      image.getBuffer(Jimp.MIME_PNG, (err, buffer) => {
        ...
        message.author.send({ content: `${message.author}, Escreva o que está escrito nesta imagem!`, files: [attachment] }).then(m => {
          console.log(m.channel.id)
        })
      })
    })
  })
  let channel = db.get(`Verify_DM${message.author.id}`)
  const main = channel.createMessageCollector(a => a.author.id === message.author.id, {
    time: 100000 * 50,
    max: 1
  })
